...
class RightConfigScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawCustomButton(graphics, "Размер (П): -" + MyModConfig.rightScalePercent + "%", cx, cy + 80, 100, 20, mouseX, mouseY);
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        if (button == 0) {
            ...
            if (mx >= cx && mx <= cx + 100) {
                if (my >= cy - 70 && my <= cy - 50) { MyModConfig.rightY += 0.05f; return true; }
                if (my >= cy - 45 && my <= cy - 25) { MyModConfig.rightY -= 0.05f; return true; }
                if (my >= cy - 20 && my <= cy) { MyModConfig.rightZ -= 0.05f; return true; }
                if (my >= cy + 5 && my <= cy + 25) { MyModConfig.rightZ += 0.05f; return true; }
                if (my >= cy + 30 && my <= cy + 50) { MyModConfig.rightX -= 0.05f; return true; }
                if (my >= cy + 55 && my <= cy + 75) { MyModConfig.rightX += 0.05f; return true; }
                if (my >= cy + 80 && my <= cy + 100) {
                    MyModConfig.rightScalePercent = (MyModConfig.rightScalePercent + 10) % 100;
                    return true;
                }
            }
        }
...
class LeftConfigScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawCustomButton(graphics, "Размер (Л): -" + MyModConfig.leftScalePercent + "%", cx, cy + 80, 100, 20, mouseX, mouseY);
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        if (button == 0) {
            ...
            if (mx >= cx && mx <= cx + 100) {
                if (my >= cy - 70 && my <= cy - 50) { MyModConfig.leftY += 0.05f; return true; }
...
class ParticleConfigScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        this.renderBackground(graphics, mouseX, mouseY, partialTick);
        graphics.drawCenteredString(this.font, "Меню выбора частиц (В разработке)", this.width / 2, this.height / 2, 0xFFFFFF);
        super.render(graphics, mouseX, mouseY, partialTick);
    }
}
